...
import { listCapturePursuits } from "@/lib/services/capture";
...
export default async function CaptureListPage() {
  const ctx = await requireGovConContext();

  let body: React.ReactNode;
  try {
    const pursuits = await listCapturePursuits(ctx);
    if (pursuits.length === 0) {
      body = (
        <Card>
          <EmptyState
            title="No pursuits in capture"
            description="Pursuits reach the capture worklist once they are qualified or have a capture plan."
          />
        </Card>
      );
    } else {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {pursuits.map((o) => {
                  const total = o.capturePlan?.sections.length ?? 0;
                  const approved = (o.capturePlan?.sections ?? []).filter(
                    (s) => s.status === "APPROVED" || s.status === "LOCKED",
                  ).length;
                  return (
                    <tr key={o.id} className="hover:bg-slate-50">
                      <td className="px-4 py-2.5">
                        <Link href={`/opportunities/${o.id}/capture`} className="font-medium text-slate-800 hover:text-blue-600">
                          {o.internalName}
                        </Link>
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-slate-600">
                        {o.capturePlan ? `${approved}/${total} approved` : "No plan"}
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <Link href={`/opportunities/${o.id}/capture`} className="text-xs text-blue-600 hover:underline">
                          Open →
                        </Link>
                      </td>
                    </tr>
    ...
    body = <ErrorState title="Capture worklist unavailable" />;
  }
  ...
  return (
    <>
      <PageHeader
        title="Capture"
        subtitle="Pursuits in active capture planning, ready to shape a winning bid."
      />
      {body}
    </>
  );
